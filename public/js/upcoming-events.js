...
let { eventsPerRow, singleRowNavigation } = loadDisplaySettings();
let generatedCode = "";
let updateTimer = 0;
...
function saveDisplaySettings() {
  try {
    localStorage.setItem(DISPLAY_STORAGE_KEY, JSON.stringify({ eventsPerRow, singleRowNavigation }));
  } catch (error) {
...
function generatedCarouselCardBasis() {
  const totalGap = Math.max(0, eventsPerRow - 1) * 18;
  return eventsPerRow === 1 ? "100%" : `calc((100% - ${totalGap}px) / ${eventsPerRow})`;
}
  ...
  try {
    const response = await fetch("/api/save-page", {
      ...
      body: JSON.stringify({
        ...
        display: { eventsPerRow, singleRowNavigation },
        code: generatedCode
      })
    });
