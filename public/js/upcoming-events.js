...
function parseGeneratedUpcomingEventsCode(code) {
  ...
  return cards.slice(0, MAX_EVENTS).map((card, index) => {
    ...
    const importedEvent = cloneEvent({
      ...
      venue: card.querySelector(".event-venue-text")?.textContent.trim() || card.querySelector(".event-venue")?.textContent.trim() || "",
...
function importPastedUpcomingCode() {
  ...
  try {
    const importedEvents = parseGeneratedUpcomingEventsCode(pastedCode);
    ...
    upcomingImportStatus.textContent = `Imported ${importedEvents.length} event${importedEvents.length === 1 ? "" : "s"} and regenerated the preview.`;
  } catch (error) {
